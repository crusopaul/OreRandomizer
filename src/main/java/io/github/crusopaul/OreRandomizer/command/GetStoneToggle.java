...
public class GetStoneToggle implements CommandExecutor, TabCompleter {
  public GetStoneToggle(OreListener oreListenerToSet) {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    if (sender.hasPermission("OreRandomizer.GetStoneToggle") || !(sender instanceof Player)) {
      final boolean toggle = validityCheckAndErrorMessage(sender, args);

      sender.sendMessage(
          ChatColor.BLUE
              + "Ore randomization is occuring upon stone creation: "
  ...
  private boolean validityCheckAndErrorMessage(CommandSender sender, String[] args) {
    return this.oreListener.getRandomizeOnStoneCreation();
  }
