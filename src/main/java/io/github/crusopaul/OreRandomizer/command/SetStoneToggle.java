...
public class SetStoneToggle implements CommandExecutor, TabCompleter {
  public SetStoneToggle(OreListener oreListenerToSet) {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    if (sender.hasPermission("OreRandomizer.SetStoneToggle") || !(sender instanceof Player)) {
      ...
      switch (toggle) {
        case 0:
        case 1:
          this.oreListener.getConfigFile().set("RandomizeOnStoneCreation", toggleBoolean);
          this.oreListener.setRandomizeOnStoneCreation(toggleBoolean);
          this.oreListener.saveConfigFile();

          sender.sendMessage(
              ChatColor.BLUE
                  + "Ore randomization upon stone creation has been set to: "
  ...
  private int validityCheckAndErrorMessage(CommandSender sender, String[] args) {
    ...
    if (args.length > 0) {
      ...
      sender.sendMessage(ChatColor.RED + "/SetStoneToggle takes one argument.");
    }
